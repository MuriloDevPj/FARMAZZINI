...
COLS = ["ean", "nome", "marca", "preco_sem_desconto", "preco_pix",
        "preco_cartao", "desconto", "disponivel",
...
def limpar_base(df, nome_farmacia):
    # Garante que todas as colunas existem (inclusive as novas)
    for col in COLS:
    ...
    # Novos campos — tratamento simples: vazio vira "NONE"
    df["cashback"]         = df["cashback"].apply(normalizar_campo)
    df["desconto_especial"] = df["desconto_especial"].apply(normalizar_campo)
...
def lambda_handler(event, context):
    try:
        ...
        # 2. Determina a farmácia pelo nome do arquivo
        ...
        df_bruto = pd.read_csv(
            io.BytesIO(response['Body'].read()), dtype=str, encoding="utf-8-sig"
        )

        # 4. Executa a limpeza
        df_limpo = limpar_base(df_bruto, nome_real)

        # 5. Converte DataFrame para Parquet em memória
